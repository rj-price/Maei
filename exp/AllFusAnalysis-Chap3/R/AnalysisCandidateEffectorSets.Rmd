...
p_nodes <- p + 
  ...
  geom_tiplab(aes(label = label), offset = 0.008) +
  geom_cladelabel(node=node_number, label="Adjusted by: 0.03", color='black') +
...
```{r TEF1a tree, results= 'hide', fig.height= 20, fig.width= 12}
...
p2 <- p +
  geom_cladelabel(node = 88, label = "Group 1", offset = 0.018) +
  geom_cladelabel(node = 80, label = "Group 2", offset = 0.02) +
  geom_cladelabel(node = 77, label = "Group 3", offset = 0.018) +
  geom_cladelabel(node = 75, label = "Group 4", offset = 0.018) +
  geom_cladelabel(node = 67, label = "Group 5", offset = 0.018) +
  geom_cladelabel(node = 64, label = "Group 6", offset = 0.018) +
  geom_cladelabel(node = 59, label = "Group 7", offset = 0.018) +
  geom_cladelabel(node = 55, label = "Group 8", offset = 0.018) +
  geom_highlight(node = 88, fill = "mistyrose") + # colour the Fs node
  geom_highlight(node = 49, fill = "lemonchiffon1" ) + # colour the fo node
  geom_treescale(x = 0, y = 1.5, width = 0.004) + 
 ...
 # geom_tiplab(aes(label = isolate_code), color = "grey20", offset = 0.0045, linetype = "blank", geom = "text", align = TRUE) +
  geom_tiplab(aes(label = race), color = "grey20", offset = 0.0239, linetype = "blank", geom = "text", align = TRUE, hjust = 1)+
  geom_tippoint(aes(shape = source), size = 2.5) +
  geom_rootedge() +
  theme(legend.position = "bottom" ) +
  geom_cladelabel(node=node_number, label="Adjusted by: 0.03", offset = 0.011, color='black') + 
...
```{r TEF1a tree plot, fig.height= 20, fig.width= 12, fig.cap="TEF phylogeny of Fusarium isolates included int effector anaylisis.The tree is rooted through F. graminearum."}
# plot it 
plot(p4)
...
sum(stats_data$mimps)
sum(stats_data$candidate_effectors)
...
stats_plot_data <- select(metadata,"species", "species_group", "fsp", "isolate_code","genome_size","no._mimps","no._cand_effs", "TefGroup") %>%
#Rename the columns to reduce the long titles
  dplyr::rename(isolate=isolate_code,                                    
         mimps=no._mimps,
         candidate_effectors=no._cand_effs,
...
First, I wanted to look at the distribution of the these clusters in numerical terms. How many CECs are there in each of the Fusarium genomes I included. To do this, I counted the total number of 1s in each columns from the bimary matrix, where each column represents a CEC. I then merged that data with the metadata df, to keep track going forward.  
...
CEC_metadata <- select(metadata,"species", "species_group", "fsp", "race", "isolate_code","genome_size","no._mimps","no._cand_effs", "no._CECs", "TefGroup")
...
Next I wanted to look at the size of the CECs, how many sequences are there in each CEC? Do any fsp have unique CECs which are expanded?
...
I wonder if Cluster234 is generally large, I'll take a look at the clusters to find out which are largest and smallest overall.
...
```
Cluster 234 is the largest overall, helped, no doubt, by lactucae. 895 sequences in that cluster seems considerable! I'm doubtful that's actually an effector - its possibly an ABC transporter which has not been filtered out by EffectorP. It tends to do that. 

There are 80 CECs with just 1 sequence, some may be genuine, some may not. 

I want to look at the distribution of the count of CEs in the CECs. 

```{r plot of CE distrib in CEC}
...
ggsave("CEC_count_plot.png")

```

The majority of the clusters have 0 or 1 CE in them, with only a handful having > 500 CEs.
...
First, I clustered the data in the binary data matrix, so that it will be ordered  when I visualise the candidate effector heatmap.
...
Next, I built the plot. I started using `pheatmap` but that didn't have all the functionality I wanted, so I swapped to `ComplexHeatmap` with the `pheatmap` plug in, so I didn't have to start the heatmap again!
...
heatmap_metadf <- select(metadata,"label","no._mimps","no._cand_effs", "no._CECs", "race", "full_name", "isolate_code", "TefGroup") %>%
...
row_anno_r = rowAnnotation(
  "Tef1a Group" = heatmap_metadf_ordered$TefGroup,
...
As apii and coriandrii are have similar genomes, and apii r4 can infect coriander, I want to examine their shared CECs more closely. 

First, how many core CECs in apii, how many core CECs shared between apii R3 and R4 and what are the CECs that are not shared?
```{r CEC counts in apii}
...
Next, I want to look at coriandrii alone. How many core CECs between coriandrii and how many core CECs when you include apii? 

```{r CEC counts in coriandrii}
...
Knowing that coriandrii 3-2 and apii race 3 and race 4 are closely related and have a similar CEC profile, how many CECs are shared between just these isolates

```{r Foa r3, r4 and coriandrii 3-2 monophyletic group CEC counts}
...
```{r Foa r2 and coriandrii AJ615 monophyletic group CEC counts}
# ---- Foa monophyletic group CEC counts ---- #
# subset only the Foa rows
...
Next, how many CECs are shared among all of the Fs and Fusarium from TNAU. 

```{r CEC count in fusarium of bana}
...
```{r banana upset plot, fig.width= 10,  height = 6}
# ---- Filter the data ---- #
...
Further, Foc1_60 displays quite a reduced CEC profile compared to the other R1 isolates, inlcuding N2 (from the same tef lineage).

I'll just quickly plot total CEC count per isolate to see if it as an outlier.


```{r plot of CEC count, fig.width= 16, fig.height= 10}
# ---- prepare the data ---- #
#identify all rows in the metadata which do not contain cubense 
...
# save it
ggsave("./CecDistribinFspOfInterest.png", width = 16, height = 10)

```

What does the distribution of clusters look like in terms of numbers? How many are shared among the differnt races of Foc? 

```{r Foc TR4 monophyletic group CEC counts}
...
```{r Foc TR4 and VPR monophyletic group CEC counts}
# ---- Foc tr4 monophyletic group CEC counts ---- #
# subset only the Foc tr4 rows
...
How are these CECs distributed across isolates?

```{r fusarium of apii and coriandrii}
...
Just to identify the differences in CECs between the Fola isolates, specifically the R4 isolates as AJ516 displays quite a different CEC profile from the other R4 assemblies. 

All Fola isolates:
```{r Fo lactucae group CEC counts}
# ---- Fola monophyletic group CEC counts ---- #
# subset only the Foc tr4 rows
heatmap_Fola_only <- subset(CEC_binary_df, grepl("Fo._fsp._lactucae", rownames(CEC_binary_df)))
  # count the number of rows where the column total is >= the number of Fo rows (a shared candidate effector cluster!)
...
```{r Fo lactucae R4 group CEC counts}
# ---- Foc tr4 monophyletic group CEC counts ---- #
# subset only the Fola r4 rows
...
# -- identify which CECs are unique 
# first drop all the columns where all values == 0 in the Fola r4 specific matrix
heatmap_Fola_only_drop_0 <- heatmap_Fola_only[, colSums(heatmap_Fola_only != 0) > 0]
#next use set diff to identify the different columns 
not_shared_cols <- setdiff(names(heatmap_Fola_only_drop_0), names(heatmap_Fola_only_shared))
```

```{r Fo lactucae R1 group CEC counts}
# ---- Foc tr4 monophyletic group CEC counts ---- #
# subset only the Fola r1 ows
heatmap_Fola_only <- subset(CEC_binary_df, grepl("Fo._fsp._lactucae_AJ718|Fo._fsp._lactucae_AJ865|Fo._fsp._lactucae_AJ520", rownames(CEC_binary_df)))
  # count the number of rows where the column total is >= the number of Fo rows (a shared candidate effector cluster!)
heatmap_Fola_only_shared <- heatmap_Fola_only %>%
  select_if(colSums(heatmap_Fola_only) >= nrow(heatmap_Fola_only))
# count the number of columns 
