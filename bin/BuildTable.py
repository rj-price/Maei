...
for infile in files_names:
    #Create the dataframe from the sequence hits in the genomes (identified in the Occurrences file input file (infile)).
        ...
        Occurrences=pd.read_csv(infile, names=header_list, delimiter='\t', index_col=False)    #Read the input file as a tab separated dataframe.
        ...
        #Trim the file path so that the column names are just the genomes.
        InName = os.path.basename(infile)   #Trim to just the file name
        Trimmed = InName.replace('Occurrences_of','') #Trim the file name preffix
